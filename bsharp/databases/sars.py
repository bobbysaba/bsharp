import numpy as np
import os
#import bsharp.io.spc_decoder as spc_decoder
...
def supercell(database_fn, mlcape, mllcl, h5temp, lr, shr, srh, shr3k, shr9k, srh3):
    ...
    quality_match_tortype = np.asarray(supercell_database[:,1][quality_match_idx], dtype=str)
    ...
    quality_match_soundings = np.array(quality_match_soundings, dtype=str)
    quality_match_tortype = np.array(quality_match_tortype, dtype=str)
...
def hail(database_fn, mumr, mucape, h5_temp, lr, shr6, shr9, shr3, srh):
    ...
    quality_match_dates = np.array(quality_match_dates, dtype=str)
...
def getSounding(match_string, match_type, profile="default"):
    ...
    datafile = None
    for file in files:
        if file.startswith(match_date) and (file.endswith(match_loc.lower()) or file.endswith(match_loc.upper())):
