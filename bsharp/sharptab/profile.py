...
from bsharp.sharptab import utils, winds, params, interp, thermo, watch_type, fire
import bsharp.io.qc_tools as qc_tools
from bsharp.databases.sars import hail, supercell
from bsharp.databases.pwv import pwv_climo
from bsharp.sharptab.constants import MISSING
import logging
import warnings
...
class Profile(object):
    ...
    def toFile(self, file_name):
        ...
        date_str = self.date.strftime("%y%m%d/%H%M") if self.date is not None else "000000/0000"
        snd_file.write("%s   %s\n Saved by user: %s on %s UTC\n" % (snd_loc, date_str, user, now.strftime('%Y%m%d/%H%M')))
...
class ConvectiveProfile(BasicProfile):
    ...
    def get_PWV_loc(self):
        ...
        None
        '''
        month = int(self.date.strftime('%m')) if self.date is not None else None
        self.pwv_flag = pwv_climo(self, self.location, month=month)
