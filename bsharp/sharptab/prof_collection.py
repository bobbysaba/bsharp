...
import bsharp.sharptab.interp as interp
import platform
import numpy as np
...
class ProfCollection(object):
    ...
    def _backgroundCopy(self, member, max_procs=2):
        """
        Copies the profile objects by upgrading them from Profile to ConvectiveProfile
        via the _target_type variable.

        member:     the key indicating a specific member
        max_procs:  unused, kept for API compatibility
        """
        for idx, prof in enumerate(self._profs[member]):
            self._profs[member][idx] = self._target_type.copy(prof)
        return
