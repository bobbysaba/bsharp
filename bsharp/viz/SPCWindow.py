...
import logging
from bsharp.viz import plotSkewT, plotHodo, plotText, plotAnalogues
...
class SPCWidget(QWidget):
    ...
    def __init__(self, **kwargs):
        ...
        self.setStyleSheet(
            "QWidget { background-color: " + bg_hex + "; color: " + fg_hex + "; }"
            "QMenuBar { background-color: " + bg_hex + "; color: " + fg_hex + "; }"
            "QMenuBar::item:selected { background-color: #2a4a7a; }"
            "QMenu { background-color: #1e1e1e; color: " + fg_hex + "; border: 1px solid #333333; }"
...
#            self.insets[inset].setParent(self.text)
    ...
    def updateConfig(self, config, update_gui=True):
        ...
        # Rebuild the full window stylesheet with updated colors
        self.setStyleSheet(
            "QWidget { background-color: " + bg_hex + "; color: " + fg_hex + "; }"
            "QMenuBar { background-color: " + bg_hex + "; color: " + fg_hex + "; }"
            "QMenuBar::item:selected { background-color: #2a4a7a; }"
...
class SPCWindow(QMainWindow):
    ...
    def testDataIntegrity(self, prof):
        try:
            ...
            logging.exception(e)
            msgBox = QMessageBox()
            msgBox.setText("bsharp has detected that the data you are attempting to load may have errors.")
