...
class Mapper(object):
    ...
    min_lat = {'npstere':0., 'merc':-85., 'spstere':-90.}
    max_lat = {'npstere':90., 'merc':85., 'spstere':0.}
...
class MapWidget(QWidget):
    ...
    def __init__(self, data_source, init_time, async_object, **kwargs):
        ...
        self.stn_readout.setStyleSheet("QLabel { background-color:transparent; border-width: 0px; font-size: 16px; color: #FFFFFF; }")
        ...
        self.load_readout.setStyleSheet("QLabel { background-color:transparent; border-width: 0px; font-size: 18px; color: #FFFFFF; }")
        ...
        self.latlon_readout.setStyleSheet("QLabel { background-color: rgba(0,0,0,140); border-radius: 4px; padding: 2px 5px; font-size: 12px; color: #e2e2e2; }")
        self.latlon_readout.setText("")
        self.latlon_readout.setFixedWidth(160)
        self.latlon_readout.show()
        self.latlon_readout.move(10, self.height() - 28)

        self.no_internet = QtWidgets.QLabel(parent=self)
        self.no_internet.setStyleSheet("QLabel { background-color:transparent; border-width: 0px; font-size: 32px; color: #FFFFFF; }")
        self.no_internet.setText("No Internet Connection")
        self.no_internet.show()
        txt_width = self.no_internet.fontMetrics().horizontalAdvance(self.no_internet.text())
    ...
    def selectStation(self, point):
        """Programmatically select a station as if the user clicked it, and center the map on it."""
        self.clicked_stn = point['srcid']
        self.clicked.emit(point)

        # Pan to center the station on screen using the current painter transform.
        # self.transform maps projection-space coords → screen-space coords and is
        # updated every drawMap() call, so it already encodes scale + rotation.
        if hasattr(self, 'transform') and len(self.stn_lats) > 0:
            proj_x, proj_y = self.mapper(point['lat'], point['lon'] + self.map_rot)
            sx, sy = self.transform.map(proj_x, proj_y)
            self.map_center_x += self.width() / 2 - sx
            self.map_center_y += self.height() / 2 - sy
            self.trans_x, self.trans_y = 0, 0
    ...
    def resetViewport(self, ctr_lat=None, ctr_lon=None):
        ...
        if ctr_lat is not None and ctr_lon is not None:
            ...
            if proj == 'npstere':
                ...
                cx, cy = self.mapper(38., self.mapper.getLambda0())
                self.map_center_y = self.height() / 2 - cy / self.scale
            elif proj == 'spstere':
                self.map_center_y = 13 * self.height() / 10 + self.height() / 2
    ...
    def mouseReleaseEvent(self, e):
        ...
        if not self.dragging and len(self.stn_lats) > 0:
            ...
            if dists[stn_idx] <= 5:
                ...
                point = self.points[idxs[stn_idx]]
                self.clicked.emit(point)
...
#       self._async.post(self.initMap, update)
    ...
    def hasInternet(self, has_connection):
        ...
        if has_connection:
            ...
            txt_width = met.horizontalAdvance(self.no_internet.text())
...
#           return
        ...
        if dists[stn_idx] <= 5:
            ...
            fm = QtGui.QFontMetrics(QtGui.QFont(self.font().family(), 16))

            label_offset = 5
            align = 0
            if stn_x > self.width() / 2:
                sgn_x = -1
                label_x = stn_x - fm.horizontalAdvance(self.stn_names[idxs[stn_idx]])
            ...
            self.stn_readout.setFixedWidth(fm.horizontalAdvance(self.stn_names[idxs[stn_idx]]))
