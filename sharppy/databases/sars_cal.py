...
def get_profile(fname, sars_type):
    ...
    # Load in the data
    try:
        sars_fname = sars.getSounding(fname[0], sars_type)
    except:
...
def check_supercell_cal(use_db=True):
    ...
    supercell_db = np.loadtxt(database_fn, skiprows=1, dtype=str, comments="%%%%")
    hits = 0
    fa = 0
    cn = 0
    miss = 0
...
def check_hail_cal(use_db=True):
    ...
    hail_db = np.loadtxt(database_fn, skiprows=1, dtype=str)
    hits = 0
    cn = 0
    miss = 0
    fa = 0 
