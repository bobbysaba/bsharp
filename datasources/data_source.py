...
def _pingURL(hostname, timeout=1):
    try:
        ...
        urlopen(hostname, timeout=timeout, context=ssl_ctx)
    except URLError:
        return False
...
class Outlet(object):
    ...
    def getAvailableAtTime(self, **kwargs):
        ...
        if self._name.lower() in available.availableat and self._ds_name.lower() in available.availableat[self._name.lower()]:
            ...
            except (URLError, IncompleteRead) as err:
                logging.warning("getAvailableAtTime %s/%s: %s", self._name, self._ds_name, err)
                stns_avail = []
                self._is_available = False
    ...
    def getAvailableTimes(self, max_cycles=100, **kwargs):
        ...
        if self._custom_avail:
            try:
                ...
                logging.warning("Outlet %s/%s unavailable: %s", self._name, self._ds_name, err)
                custom_failed = True
                self._is_available = False
    ...
    def hasProfile(self, point, cycle):
        ...
        logging.debug(f"hasProfile outlet={self._name!r} cycle={cycle!r} → {has_prof}")
        return has_prof
...
class DataSource(object):
    ...
    def _getOutletWithProfile(self, stn, cycle_dt, outlet_num=0):
        outlets = list(self._outlets.keys())
        try:
            return outlets[outlet_num]
        except IndexError:
    ...
    def getURL(self, stn, cycle_dt, outlet_num=0, outlet=None):
        ...
        if self._name == 'NUCAPS CONUS NOAA-20' \
            ...
            fmt = {
                'srcid':quote(stn['srcid']),
                'synop':quote(str(stn.get('synop', stn['srcid']))),
