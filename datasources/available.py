...
def _download_goes():
    ...
    if goes_time is None or goes_time < now - cache_len:
        url_obj = urlopen(goes_base_url, context=_ssl_ctx)
...
def _availableat_goes(dt):
    ...
    text = urlopen(recent_url, context=_ssl_ctx).read().decode('utf-8')
    matches = re.findall(r"(.+).txt", text)
    return matches
...
def _download_sharp():
    ...
    if sharp_time is None or sharp_time < now - cache_len:
        url_obj = urlopen(sharp_base_url, context=_ssl_ctx)
...
def _download_sharp_archive(dt):
    ...
    if sharp_archive_time is None or sharp_archive_time < now - cache_len:
        url_obj = urlopen(dt.strftime(base_url), context=_ssl_ctx)
...
def _availableat_sharp(dt):
    ...
    #matches = re.findall(r"a href=\"(.+).txt\"", text)
    recent_url = 'http://sharp.weather.ou.edu/soundings/archive/%Y/%m/%d/%H/'
    text = urlopen(dt.strftime(recent_url), context=_ssl_ctx).read().decode('utf-8')
    matches = re.findall(r"a href=\"(.+).txt\"", text)
    return matches
...
def _download_nucaps_conus_j01():
    global nucaps_text
    nucaps_text = urlopen(nucaps_conus_j01_url, context=_ssl_ctx).read().decode('utf-8')
    return nucaps_text
...
def _download_nucaps_conus_aq0():
    global nucaps_text
    nucaps_text = urlopen(nucaps_conus_aq0_url, context=_ssl_ctx).read().decode('utf-8')
    return nucaps_text
...
def _download_nucaps_conus_m01():
    global nucaps_text
    nucaps_text = urlopen(nucaps_conus_m01_url, context=_ssl_ctx).read().decode('utf-8')
    return nucaps_text
...
def _download_nucaps_conus_m02():
    global nucaps_text
    nucaps_text = urlopen(nucaps_conus_m02_url, context=_ssl_ctx).read().decode('utf-8')
    return nucaps_text
...
def _download_nucaps_conus_m03():
    global nucaps_text
    nucaps_text = urlopen(nucaps_conus_m03_url, context=_ssl_ctx).read().decode('utf-8')
    return nucaps_text
...
def _download_nucaps_caribbean_j01():
    global nucaps_text
    nucaps_text = urlopen(nucaps_caribbean_j01_url, context=_ssl_ctx).read().decode('utf-8')
    return nucaps_text
...
def _download_nucaps_alaska_j01():
    global nucaps_text
    nucaps_text = urlopen(nucaps_alaska_j01_url, context=_ssl_ctx).read().decode('utf-8')
    return nucaps_text
...
spc_base_url = "https://www.spc.noaa.gov/exper/soundings/"
spc_text = ""
spc_time = None
_spc_at_cache = {}       # {datetime: (fetch_time, [station_ids])}
_spc_at_cache_len = timedelta(minutes=30)
...
def _download_spc():
    ...
    if spc_time is None or spc_time < now - cache_len:
        try:
            url_obj = urlopen(spc_base_url, context=_ssl_ctx)
            spc_text = url_obj.read().decode('utf-8')
            spc_time = now
        except IncompleteRead as e:
            # Server returned partial chunked response — use whatever was downloaded
            spc_text = e.partial.decode('utf-8')
            spc_time = now
        except Exception:
            # Other network error — return cached text if available, else re-raise
            if not spc_text:
                raise
...
def _available_spc(dt=None):
    ...
    '''
    text = _download_spc()
    matches = sorted(list(set(re.findall(r"([\d]{8})_OBS", text))))
    return [ datetime.strptime(m, '%y%m%d%H') for m in matches ]
...
def _availableat_spc(dt):
    '''
        ...
        matches : array of strings
            An array that contains all of the three letter station identfiers.
    '''
    global _spc_at_cache
    now = _utcnow()
    if dt in _spc_at_cache:
        fetch_time, cached = _spc_at_cache[dt]
        if now - fetch_time < _spc_at_cache_len:
            return cached

    recent_url = "%s%s/" % (spc_base_url, dt.strftime('%y%m%d%H_OBS'))
    try:
        url_obj = urlopen(recent_url, context=_ssl_ctx)
        text = url_obj.read().decode('utf-8')
    except IncompleteRead as e:
        text = e.partial.decode('utf-8')

    matches = re.findall(r"show_soundings\(\"([\w]{3}|[\d]{5})\"\)", text)
    _spc_at_cache[dt] = (now, matches)
    return matches
...
def _download_psu():
    ...
    if psu_time is None or psu_time < now - cache_len:
        url_obj = urlopen(psu_base_url, context=_ssl_ctx)
...
def _availableat_psu(model, dt):
    ...
    url_obj = urlopen(url, context=_ssl_ctx)
    text = url_obj.read().decode('utf-8')
...
def _available_psu(model, dt=None):
    ...
    if dt is not None and dt < _utcnow() - timedelta(3600*29): # We know PSU is only a 24 hr service
        return []
...
def _download_iem():
    ...
    if iem_time is None or iem_time < now - cache_len:
        iem_obj = urlopen(iem_base_url, context=_ssl_ctx)
        psu_text = url_obj.read().decode('utf-8')
        iem_time = now
...
def _available_iem(model, dt=None):
    ...
    if dt < datetime(2010,12,30): # No Bufkit files before this date
        return []
    if dt > _utcnow() - timedelta(seconds=3600*24):
        return []
...
def _available_oupecan(**kwargs):
    text = urlopen(pecan_base_url, context=_ssl_ctx).read().decode('utf-8')
    matches = sorted(list(set(re.findall(r"([\d]{10})", text))))
    return [ datetime.strptime(m, "%Y%m%d%H") for m in matches ]
...
def _available_ncarens(dt=None):
    text = urlopen(ncarens_base_url, context=_ssl_ctx).read().decode('utf-8')
